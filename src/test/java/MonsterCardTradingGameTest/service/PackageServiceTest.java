...
public class PackageServiceTest {
    ...
    void createPackageTestNoAdmin(){
        ...
        card.setDamage(25);
        cards.add(card);
        String token = "skdjb-mtcgToken";


        DataAccessException exception = assertThrows(DataAccessException.class, () -> {
            packageRepository.createPackage(cards,token);
            packageRepositoryTest.getCreatedPackageTest();
        });

        assertTrue(exception.getMessage().contains("you are not an admin"));
